...
## 2026-05-20 16:05:00 +0800
- Summary: Fixed the likely GPIO controller non-response path by making `gamepad_cursor.py` and `hud_overlay.py` prefer the `Xiphias GPIO Gamepad` device and reject unrelated key-plus-axis devices such as touch hardware. The GPIO gamepad service now self-loads `uinput` before starting, and `check_gpio_gamepad.sh` reports service state, `/dev/uinput`, Python dependencies, input devices, udev gamepad tags, and recent logs for on-device troubleshooting.
- Files: `release/gamehub-console/gamepad_cursor.py`, `release/gamehub-console/hud_overlay.py`, `release/gamehub-console/files/xiphias-gpio-gamepad.service`, `release/gamehub-console/check_gpio_gamepad.sh`, `release/gamehub-console/install_gamehub_console.sh`, `release/gamehub-console/README.md`, `release/gamehub-console/xiphias-changes.md`, `release/gamehub-console/timestamp-console.md`.
- Verification: Ran `python -m py_compile release/gamehub-console/gpio_gamepad.py release/gamehub-console/gamepad_cursor.py release/gamehub-console/hud_overlay.py` and `git diff --check` in this checkout. `bash -n` could not be run because this Windows workspace does not have `bash` installed. Final on-device verification still requires running `bash /home/pi/Xiphias/release/gamehub-console/check_gpio_gamepad.sh` on the Raspberry Pi and pressing the connected GPIO buttons in `evtest`.

## 2026-05-20 15:38:07 +0800
- Summary: Tightened the GPIO controller bridge so the custom GPIO controls present more like a generic USB controller. The virtual device now uses USB-style uinput identity, advertises right-stick and trigger axes, emits L2/R2 as both digital trigger buttons and trigger axes, and installs a udev rule that tags `Xiphias GPIO Gamepad` as a joystick/gamepad for Chromium and OS-level consumers.
- Files: `release/gamehub-console/gpio_gamepad.py`, `release/gamehub-console/files/99-xiphias-gpio-gamepad.rules`, `release/gamehub-console/install_gamehub_console.sh`, `release/gamehub-console/ota_git_update.sh`, `release/gamehub-console/README.md`, `release/gamehub-console/AGENTS.md`, `release/gamehub-console/xiphias-changes.md`, `release/gamehub-console/timestamp-console.md`.
- Verification: Ran `python -m py_compile release/gamehub-console/gpio_gamepad.py release/gamehub-console/gamepad_cursor.py release/gamehub-console/hud_overlay.py` in this checkout. `bash -n` could not be run because this Windows workspace does not have `bash` installed. Final on-device verification still requires a Raspberry Pi with the GPIO buttons connected to confirm udev reports `ID_INPUT_JOYSTICK=1` and `ID_INPUT_GAMEPAD=1`, Chromium sees the gamepad, and L2/R2 register as trigger axes.

## 2026-05-20 15:14:48 +0800
- Summary: Added native Xiphias external GPIO gamepad support for the handheld wiring from Leandro Linares' guide. The new root-run `gpio_gamepad.py` service reads the default BCM GPIO buttons, publishes a virtual `Xiphias GPIO Gamepad` evdev device, loads `uinput`, and integrates with the existing cursor and Settings overlay paths without emitting browser-visible keyboard letters.
- Files: `release/gamehub-console/gpio_gamepad.py`, `release/gamehub-console/files/xiphias-gpio-gamepad.service`, `release/gamehub-console/files/uinput.conf`, `release/gamehub-console/gamepad_cursor.py`, `release/gamehub-console/hud_overlay.py`, `release/gamehub-console/install_gamehub_console.sh`, `scripts/build_xiphias_image.sh`, `release/gamehub-console/ota_git_update.sh`, `publish/public-root/gamehub-console/console.env`, `release/gamehub-console/README.md`, `release/gamehub-console/AGENTS.md`, `release/gamehub-console/xiphias-changes.md`, `release/gamehub-console/timestamp-console.md`.
- Verification: Ran `python -m py_compile release/gamehub-console/gpio_gamepad.py release/gamehub-console/gamepad_cursor.py release/gamehub-console/hud_overlay.py` in this checkout. `bash -n` could not be run because this Windows workspace does not have `bash` installed. Final on-device verification still requires a Raspberry Pi with the GPIO buttons connected to confirm the service creates `Xiphias GPIO Gamepad`, D-pad cursor movement does not scroll Chromium, `Start` opens Settings, `A`/`B` operate Settings, `Y` toggles the OSK, and shoulder/trigger scrolling works.
...
## 2026-05-20 16:17:08 +0800
- Summary: Strengthened the GPIO controller path for browser gamepad support by loading `joydev` alongside `uinput`, so the virtual `Xiphias GPIO Gamepad` can expose a `/dev/input/js*` joystick node in addition to evdev. Expanded the Pi-side GPIO check script so it reports configured BCM pins, live pressed/released pin states, joystick nodes, udev tags, service state, dependencies, and recent logs.
- Files: `release/gamehub-console/files/uinput.conf`, `release/gamehub-console/files/xiphias-gpio-gamepad.service`, `release/gamehub-console/check_gpio_gamepad.sh`, `release/gamehub-console/README.md`, `release/gamehub-console/AGENTS.md`, `release/gamehub-console/xiphias-changes.md`, `release/gamehub-console/timestamp-console.md`.
- Verification: Ran `bash -n release/gamehub-console/check_gpio_gamepad.sh release/gamehub-console/install_gamehub_console.sh release/gamehub-console/ota_git_update.sh scripts/build_xiphias_image.sh` and `py -3 -m py_compile release/gamehub-console/gpio_gamepad.py release/gamehub-console/gamepad_cursor.py release/gamehub-console/hud_overlay.py release/gamehub-console/common.py` from the Windows workspace. Final hardware confirmation still requires applying the updated service/module files on the Raspberry Pi, running `sudo bash /home/pi/Xiphias/release/gamehub-console/check_gpio_gamepad.sh`, pressing each GPIO button to verify the live pin snapshot changes, confirming `/dev/input/js*` appears for the `Xiphias GPIO Gamepad`, and restarting the kiosk.

## 2026-07-09 17:22:23 +0800
- Summary: Added a runtime Settings `Button Tester` row after `Bluetooth`, with a full-height tester detail panel that draws a controller diagram, listens to USB and `Xiphias GPIO Gamepad` evdev devices without grabbing them, highlights live controls, and exits only when `Start` is held for about 1.5 seconds.
- Files: `release/gamehub-console/hud_overlay.py`, `release/gamehub-console/settings_gui.py`, `release/gamehub-console/README.md`, `release/gamehub-console/AGENTS.md`, `release/gamehub-console/xiphias-changes.md`, `release/gamehub-console/timestamp-console.md`.
- Verification: Ran `python3 -m py_compile release/gamehub-console/hud_overlay.py release/gamehub-console/settings_gui.py` from the Windows workspace, ran an LF-normalized `bash -n` check for `release/gamehub-console/restart_kiosk.sh` and `release/gamehub-console/start_kiosk_components.sh`, and ran `git diff --check`. Hardware confirmation still requires opening Settings on the Raspberry Pi, selecting `Button Tester`, testing each GPIO and USB gamepad control, confirming short `A`/`B`/`Start` presses do not trigger normal Settings actions while active, and holding `Start` to close the tester.

## 2026-07-15 10:33:27 +0800
- Summary: Added a browser game-mode gate so Xiphias stops translating controller events into mouse clicks, scrolling, OSK toggles, home actions, or the `Start` Settings shortcut while a hosted HTML game iframe such as MakeCode Arcade is active, letting USB and GPIO gamepads pass through to Chromium's Gamepad API without duplicate Xiphias actions.
- Files: `release/gamehub-console/common.py`, `release/gamehub-console/gamepad_cursor.py`, `release/gamehub-console/hud_overlay.py`, `release/gamehub-console/README.md`, `release/gamehub-console/AGENTS.md`, `release/gamehub-console/xiphias-changes.md`, `release/gamehub-console/timestamp-console.md`, `release/backup/AGENTS-20260715-103327.md`.
- Verification: Ran `python3 -m py_compile release/gamehub-console/common.py release/gamehub-console/gamepad_cursor.py release/gamehub-console/hud_overlay.py` from the Windows workspace; ran `git diff --check`. Final on-device confirmation still requires launching a hosted MakeCode/HTML game on the Raspberry Pi, pressing each GPIO control, confirming the browser game receives the input, and confirming Xiphias does not also click, scroll, toggle the OSK, open Settings from `Start`, or go home while gameplay is active.

## 2026-07-15 11:00:29 +0800
- Summary: Marked the current console build as `Alpha v1.0.0`, updated Settings and the standalone preview to preserve full alpha version labels, and added `scripts/bump_version.py` plus `.githooks/pre-commit` so normal commits can auto-increment the patch number in `version.txt`.
- Files: `version.txt`, `.githooks/pre-commit`, `scripts/bump_version.py`, `release/gamehub-console/common.py`, `release/gamehub-console/hud_overlay.py`, `release/gamehub-console/settings_gui.py`, `release/gamehub-console/README.md`, `release/gamehub-console/AGENTS.md`, `release/gamehub-console/xiphias-changes.md`, `release/gamehub-console/timestamp-console.md`, `release/backup/AGENTS-20260715-110029.md`.
- Verification: Ran `python3 -m py_compile scripts/bump_version.py release/gamehub-console/common.py release/gamehub-console/hud_overlay.py release/gamehub-console/settings_gui.py`; ran `python3 scripts/bump_version.py --dry-run` and confirmed the next patch would be `Alpha v1.0.1`; verified `format_software_version_label()` renders `1.0.0`, `v1.0.0`, and `Alpha v1.0.0` as `Alpha v1.0.0`; ran `bash -n .githooks/pre-commit`; ran `bash .githooks/pre-commit` with no staged changes; set this checkout's `core.hooksPath` to `.githooks`; and ran `git diff --check`. Final on-device confirmation still requires opening Settings and confirming the `Check for Updates` row shows `Alpha v1.0.0`.

## 2026-07-15 11:21:14 +0800
- Summary: Diagnosed the MakeCode Arcade GPIO-button failure with Playwright: the hosted portal keeps a top-level Gamepad API listener active while the MakeCode iframe is mounted, and browser button index `8` exits the iframe as `back` while MakeCode maps button `10` to reset. Updated `gpio_gamepad.py` so the virtual GPIO controller keeps the `Xiphias GPIO Gamepad` name but uses an Xbox-compatible browser identity and emits explicit D-pad button events, avoiding Chromium raw Linux button indexes for hosted HTML games.
- Files: `release/gamehub-console/gpio_gamepad.py`, `release/gamehub-console/README.md`, `release/gamehub-console/AGENTS.md`, `release/gamehub-console/xiphias-changes.md`, `release/gamehub-console/timestamp-console.md`, `release/backup/AGENTS-20260715-112114.md`.
- Verification: Used Playwright against `https://handheld.knfstudios.com/?mode=handheld` to open Guest mode, launch Pacmat Western in MakeCode Arcade, confirm the iframe loads without JS errors, confirm a mocked standard button `8` removes the game iframe as the portal `back` action while buttons `0-3`, `9`, and `12-15` keep the iframe mounted, and inspect the loaded portal/MakeCode scripts for their Gamepad API mappings. Ran `python3 -m py_compile release/gamehub-console/gpio_gamepad.py`; ran `bash -n release/gamehub-console/check_gpio_gamepad.sh release/gamehub-console/install_gamehub_console.sh release/gamehub-console/ota_git_update.sh`; ran `git diff --check`. On-device confirmation still requires reinstalling/restarting `xiphias-gpio-gamepad.service`, opening Chromium DevTools or a browser Gamepad API test page, confirming `navigator.getGamepads()[0].mapping` is `standard`, confirming buttons `0-3`, `8`, `9`, and `12-15` line up with A/B/X/Y, Select, Start, and D-pad, then launching MakeCode Arcade and pressing every GPIO control.

## 2026-07-15 11:55:43 +0800
...
## 2026-07-15 14:16:41 +0800
- Summary: Reworked Settings Wi-Fi and Bluetooth details into a two-step controller flow: the main list highlights networks/devices, D-pad right opens a nested action panel for Connect/Pair, Disconnect, and Forget actions, and D-pad left or B returns to the main list.
- Files: `release/gamehub-console/hud_overlay.py`, `release/gamehub-console/README.md`, `release/gamehub-console/AGENTS.md`, `release/gamehub-console/xiphias-changes.md`, `release/gamehub-console/timestamp-console.md`, `release/backup/AGENTS-20260715-141641.md`.
- Verification: Ran `python3 -m py_compile release/gamehub-console/hud_overlay.py`; ran `git diff --check`. On-device confirmation still requires Settings > Wi-Fi and Settings > Bluetooth navigation with D-pad right/left.
